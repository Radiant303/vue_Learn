<template>
    <div>
        ...
        <h3>当前学校为{{school}}</h3>
        <h3>当前学科为{{subject}}</h3>
        <h3>当前人员总数为{{personList.length}}</h3>
        <select v-model="n">
...
export default {
    ...
    computed: {
        ...
        ...mapGetters(['bigSum']),
        //使用mapState辅助函数,将state中的personList映射到组件的计算属性中(数组写法)
        ...mapState(['personList']),
    },
    methods: {
        //使用mapMutations辅助函数,将mutations中的JIA,JIAN映射到组件的方法中(数组写法)
        ...mapMutations(['JIA','JIAN']),

        //使用mapActions辅助函数,将actions中的jiaOdd,jiaWait映射到组件的方法中(数组写法)
